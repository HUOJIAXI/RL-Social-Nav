...
/**
 * @brief Social MPC controller with SARL (Socially Attentive RL) integration.
 *
 * Extends the VLM MPC controller with:
 *  - SARL attention-weighted social cost (replaces uniform inverse-distance)
 *  - SARL terminal value cost via batch service call
 *  - Combined VLM + SARL navigation explanation publisher
 *
 * Falls back to uniform social cost if SARL bridge is not running.
 */
...
public:
      ...
      declare_parameter<bool>("log_social_contract_to_csv", true))
  {
    ...
    w_sarl_terminal_ = declare_parameter<double>("w_sarl_terminal", 0.5);
    ...
    sarl_sub_ = create_subscription<social_mpc_nav::msg::SARLOutput>(
      ...
      std::bind(&MPCControllerNode::onSARLOutput, this, std::placeholders::_1));

    // SARL batch evaluation service client
    sarl_client_ = create_client<social_mpc_nav::srv::EvaluateSARLBatch>(sarl_service_name_);

    // Navigation explanation publisher
...
private:
  ...
  void controlLoop()
  {
    ...
    if (dist_to_final_goal < goal_tolerance_)
    {
      if (!goal_reached_)
      {
        RCLCPP_INFO(
          get_logger(),
          "GOAL REACHED! Final distance: %.3fm - Stopping robot", dist_to_final_goal);
        goal_reached_ = true;
      }
    ...
    // ========== SARL BATCH SERVICE CALL ==========
    // Call SARL batch evaluation for terminal values before solving MPC
    callSARLBatchService(robot, people, contract);
    // ========== END SARL BATCH SERVICE CALL ==========
    ...
    // ========== END PUBLISH NAVIGATION EXPLANATION ==========
    ...
    if (enable_debug_logging_)
    {
      if (tracking_waypoint)
      {
        ...
        RCLCPP_INFO(
          ...
          "v=%.2f w=%.2f | Final goal: (%.2f, %.2f) %.2fm",
      ...
      else
      {
        RCLCPP_INFO(
          ...
          "v=%.2f w=%.2f | Contract: v_max=%.2f w_goal=%.2f w_social=%.2f",
          target_x, target_y, robot.x, robot.y, dist_to_target,
          best_control->first, best_control->second,
    ...
    if (safety_check_enabled_ && scan && isCollisionImminent(scan, cmd.linear.x))
    {
      ...
      RCLCPP_WARN(
        get_logger(),
        "COLLISION IMMINENT! Obstacle at %.2fm ahead - EMERGENCY STOP", front_dist);
      cmd.linear.x = 0.0;
      cmd.angular.z = 0.0;
    }
    else
    {
      ...
      if (dist_to_final_goal < 0.5)
      {
        RCLCPP_INFO_THROTTLE(
          get_logger(), *get_clock(), 1000,
          "Approaching goal: %.2fm remaining | v=%.2f m/s, w=%.2f rad/s",
      ...
      else
      {
        RCLCPP_INFO_THROTTLE(
          ...
          "Navigating to goal: %.2fm remaining | v=%.2f m/s, w=%.2f rad/s",
          dist_to_final_goal, cmd.linear.x, cmd.angular.z);
      }
    }
  ...
  /**
   * @brief Call SARL batch evaluation service to get terminal values for all rollouts.
   *
   * Generates rollout terminal states by rolling out random control sequences,
   * packs them into the service request with current people states, and stores
   * the returned V(s) values for use in evaluateSequence().
   */
  void callSARLBatchService(
    const RobotState & robot,
    const social_mpc_nav::msg::People2D::SharedPtr & people,
    const SocialContract & contract)
  {
    sarl_terminal_values_.clear();

    // Only call if service is available
    if (!sarl_client_ || !sarl_client_->service_is_ready())
    {
      return;
    }

    // Generate rollout terminal states by simulating forward
    ...
    request->num_rollouts = num_rollouts_;
    request->goal_x = static_cast<float>(goal_x_);
    request->goal_y = static_cast<float>(goal_y_);

    // Reserve space for flattened robot states: 5 floats per rollout (x, y, yaw, vx, vy)
    request->robot_states.reserve(static_cast<size_t>(num_rollouts_) * 5);

    // Use a separate RNG so we don't disturb the main MPC RNG sequence.
    // We seed it from the main RNG to keep reproducibility.
    std::mt19937 batch_rng(rng_());

    for (int rollout = 0; rollout < num_rollouts_; ++rollout)
    {
      // Simulate forward to get terminal state
      RobotState pred = robot;
      double last_v = 0.0;
      for (int k = 0; k < horizon_steps_; ++k)
      {
        double v = vel_dist(batch_rng);
        double w = omega_dist(batch_rng);
      ...
      // Pack terminal state: [x, y, yaw, vx, vy]
      // vx, vy approximated from last velocity and heading
      request->robot_states.push_back(static_cast<float>(pred.x));
      request->robot_states.push_back(static_cast<float>(pred.y));
      request->robot_states.push_back(static_cast<float>(pred.yaw));
      request->robot_states.push_back(static_cast<float>(last_v * std::cos(pred.yaw)));
      request->robot_states.push_back(static_cast<float>(last_v * std::sin(pred.yaw)));
    }

    // Pack current people states: [x, y, vx, vy] per person
    if (people && !people->people.empty())
    {
      request->num_people = static_cast<int32_t>(people->people.size());
      request->people_states.reserve(people->people.size() * 4);
      for (const auto & person : people->people)
      {
    ...
    if (status == std::future_status::ready)
    {
      ...
      if (response->success &&
          static_cast<int>(response->values.size()) == num_rollouts_)
      {
        sarl_terminal_values_.assign(response->values.begin(), response->values.end());
        RCLCPP_DEBUG(get_logger(),
                     "SARL batch evaluation: %zu values in %.1fms",
                     sarl_terminal_values_.size(), response->inference_time_ms);
      }
      else
      {
        RCLCPP_DEBUG(get_logger(), "SARL batch evaluation returned invalid response");
      }
    }
    else
    {
      RCLCPP_DEBUG_THROTTLE(get_logger(), *get_clock(), 5000,
                            "SARL batch evaluation timed out (50ms)");
    }
  }

  /**
   * @brief Publish combined VLM + SARL navigation explanation.
   */
  void publishNavigationExplanation(const SocialContract & contract)
  {
    ...
    // Fill VLM fields
    {
      ...
      {
        const auto & vlm = latest_vlm_params_;
        explanation.scene_type = vlm->scene_type;
        explanation.crowd_density = vlm->crowd_density;
        explanation.recommended_action = vlm->recommended_action;
        explanation.vlm_confidence = vlm->confidence;
      }
      else
      {
    ...
    // Compute person distances from robot
    {
      std::lock_guard<std::mutex> lock(data_mutex_);
      if (latest_people_ && !latest_people_->people.empty() && latest_odom_)
      {
        double rx = latest_odom_->pose.pose.position.x;
        double ry = latest_odom_->pose.pose.position.y;
        for (const auto & person : latest_people_->people)
        {
          float dist = static_cast<float>(
            std::hypot(rx - person.x, ry - person.y));
    ...
    if (sarl_valid) {
      oss << " | SARL active: V(s)=" << std::fixed << std::setprecision(2)
          << explanation.state_value
          << ", attention_mult=" << scene_mult.attention_mult;
      if (!explanation.attention_weights.empty()) {
        // Find most attended person
        size_t max_idx = 0;
        float max_w = 0.0f;
    ...
    oss << " | Action: " << explanation.recommended_action
        << " | v_max=" << std::setprecision(2) << contract.v_max;
    ...
    const SocialContract & contract)
  {
    ...
    for (int rollout = 0; rollout < num_rollouts_; ++rollout)
    {
      ...
      const double cost = evaluateSequence(
        robot, people, scan, contract, v_seq, w_seq, rollout);
    ...
    int rollout_idx)
  {
    ...
    bool use_sarl = sarl_data && sarl_data->is_valid &&
        (now() - rclcpp::Time(sarl_data->stamp)).seconds() < sarl_staleness_sec_;

    // Compute scene multiplier from VLM params
    social_mpc_nav::sarl_helpers::SceneMultiplier scene_mult{1.0, 0.5};
    ...
    if (vlm_params) {
      scene_mult = social_mpc_nav::sarl_helpers::getSceneMultiplier(
        vlm_params->scene_type, vlm_params->crowd_density);
    }
    ...
    for (size_t k = 0; k < v_seq.size(); ++k)
    {
      ...
      // Social cost - SARL attention-weighted or uniform fallback
      if (people)
      {
        if (use_sarl) {
          cost += social_mpc_nav::sarl_helpers::computeAttentionWeightedSocialCost(
            pred.x, pred.y, *people, *sarl_data,
            contract.w_social, scene_mult.attention_mult);
        } else {
          cost += social_mpc_nav::sarl_helpers::computeUniformSocialCost(
            pred.x, pred.y, *people, contract.w_social);
        }

        // VLM personal distance violation cost (kept as-is)
        if (vlm_params) {
          for (const auto & person : people->people)
          {
            ...
            cost += social_mpc_nav::vlm_helpers::computeVLMPersonalDistanceCost(
      ...
      if (scan)
      {
        ...
        // HARD SAFETY CONSTRAINT: Reject trajectory if it violates minimum distance
        // This ensures the robot CANNOT get closer than hard_min_obstacle_distance_
        if (obstacle_dist < hard_min_obstacle_distance_)
        {
          return std::numeric_limits<double>::infinity();  // Reject trajectory completely
        }

        // Soft constraints for obstacle avoidance (VLM-enhanced)
    ...
    // ========== SARL TERMINAL VALUE COST ==========
    if (use_sarl && rollout_idx >= 0 &&
        static_cast<size_t>(rollout_idx) < sarl_terminal_values_.size())
    {
      cost += social_mpc_nav::sarl_helpers::computeSARLTerminalCost(
        sarl_terminal_values_[static_cast<size_t>(rollout_idx)],
        w_sarl_terminal_, scene_mult.terminal_mult);
    }
    // ========== END SARL TERMINAL VALUE COST ==========
    ...
    if (time_elapsed > 10.0 && progress < 0.5 && dist_to_goal > 1.0)
    {
      ...
      if (stuck_counter_ > 100)
      {
        RCLCPP_WARN_THROTTLE(
          ...
          "LOCAL MINIMA DETECTED! No progress in %.1fs (progress: %.2fm). "
    ...
    if (position_history_.size() >= 50)  // Require more history (5 seconds at 10Hz)
    {
      ...
      if (max_radius < 0.5 && dist_to_goal > 3.0 && progress < -0.5)
      {
        ...
        if (circle_counter_ > 100)
        {
          RCLCPP_WARN_THROTTLE(
            get_logger(), *get_clock(), 10000,
            "CIRCULAR MOTION DETECTED! Moving in %.2fm radius, goal is %.2fm away. "
  ...
  double hard_min_obstacle_distance_{0.05};
  ...
  std::vector<float> sarl_terminal_values_;
