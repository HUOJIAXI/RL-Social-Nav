...
\begin{frame}{Three-Layer Hierarchical Architecture}
  ...
  \begin{tikzpicture}[
    ...
    \node[font=\footnotesize, text width=12cm, align=center] at (0,2.0) {
      ...
      \highlight{State Value $V(s)$} \textbar{}
      \highlight{Recommended Action $(v_x, v_y)$}
    };

    % Layer 3: MPC (bottom)
    \node[layer, fill=mpcred!10, draw=mpcred!60] (mpc) at (0,0.2) {};
    \node[lbl, text=mpcred!80!black] at (-5, 0.7) {Layer 3: CasADi/IPOPT MPC};
    \node[font=\footnotesize, text width=12cm, align=center] at (0,0.0) {
      Nonlinear optimization (IPOPT) $\;\longrightarrow\;$
      \mpchl{Velocity $v^*$} \textbar{} \mpchl{Angular vel.\ $\omega^*$}
      \textbar{} \mpchl{Hard safety constraints}
    };
...
\begin{frame}{CasADi/IPOPT MPC: Problem Formulation}
  \begin{block}{Unicycle Dynamics (Symbolic, Built Once)}
    \vspace{-6pt}
    \begin{align}
  ...
  \vspace{2pt}
  \begin{block}{Nonlinear Program (Solved by IPOPT)}
    \vspace{-4pt}
    \[
  ...
  \vspace{2pt}
  \textbf{Hard Constraints (enforced by IPOPT):}
  \begin{columns}[T]
    \begin{column}{0.45\textwidth}
      \begin{itemize}
    ...
    \begin{column}{0.45\textwidth}
      \begin{itemize}
        ...
        \item Unicycle dynamics (equality)
      \end{itemize}
    \end{column}
  \end{columns}
  ...
  \centering
  \footnotesize
  $N = 15$ steps, $\Delta t = 0.2$\,s (horizon = 3.0\,s),
  CasADi symbolic graph built once $\to$ IPOPT solves at 10\,Hz
\end{frame}
...
\begin{frame}{MPC Cost Function: Full Breakdown}
  ...
  \[
    ...
    + \underbrace{J_k^{\text{smooth}}}_{\text{\mpchl{Smooth}}}
    + \underbrace{J_k^{\text{vlm}}}_{\text{\vlmhl{VLM}}}
    + \underbrace{J_k^{\text{ref}}}_{\text{\synhl{SARL Ref}}}
  \]
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \vspace{2pt}
      \textbf{SARL Action Reference} {\footnotesize (first $K{=}3$ steps):}
      \[
        J_k^{\text{ref}} = e^{-k \cdot \lambda}\bigl[
          w_v^{\text{ref}}(v_k - \hat{v})^2
          + w_\theta^{\text{ref}}(1 - \cos(\theta_k - \hat{\theta}))
        \bigr]
      \]
      {\footnotesize $\hat{v}, \hat{\theta}$ = SARL recommended speed \& heading\\
       $\lambda{=}0.5$: exponential decay over horizon}
    ...
    \begin{column}{0.48\textwidth}
      \textbf{Obstacle Cost} {\footnotesize (hard constraint in IPOPT):}
      \[
        d_{\text{obs}}(\mathbf{x}_k) \ge d_{\text{hard}} \quad \forall\, k
      \]
      {\footnotesize Plus soft barrier: $w_o / (d + 0.1)$ for $d \ge d_{\min}$}

      \vspace{6pt}
      \textbf{Terminal V(s) via Linear Approximation:}
      \[
        \boxed{J_N^{\text{term}} = -w_t \cdot m_{\text{term}} \cdot \hat{V}(s_N)}
      \]
      \vspace{-4pt}
      \[
        \hat{V}(s_N) \approx V_0 + \frac{\partial V}{\partial x}\Delta x
        + \frac{\partial V}{\partial y}\Delta y
      \]
      {\footnotesize Gradient from 5-point finite differences via batch service.\\
       $V(s)$ is a black-box NN --- linearization makes it\\
       compatible with CasADi's symbolic graph.}
    \end{column}
  \end{columns}
\end{frame}

% ============================================================
\begin{frame}{CasADi/IPOPT Optimization Pipeline with SARL}
  \centering
  \begin{tikzpicture}[
    scale=0.68, transform shape,
    box/.style={draw, rounded corners=4pt, minimum height=0.9cm,
                align=center, font=\footnotesize, line width=0.8pt},
    proc/.style={box, fill=mpcred!10, draw=mpcred!60, minimum width=3.5cm},
    sarl/.style={box, fill=sarlblue!12, draw=sarlblue!60, minimum width=3.5cm},
    setup/.style={box, fill=gray!10, draw=gray!50, minimum width=3.5cm},
    arrow/.style={-{Stealth[length=5pt]}, thick},
  ]
    % Row 1: Setup (once)
    \node[setup] (build) at (0,2.5) {\textbf{Build Once}\\
      CasADi symbolic graph\\
      dynamics + costs + constraints};

    \node[setup] (solver) at (6,2.5) {IPOPT solver config\\
      warm start, 200 iter max\\
      linear solver: MA27};

    \draw[arrow, gray] (build) -- (solver);

    % Row 2: Per-cycle parameter update
    \node[sarl] (grad) at (0,0) {$\nabla V(s_N)$ Gradient\\
      5-point finite diff.\\
      via batch service};

    \node[sarl] (ref) at (6,0) {SARL Action Ref\\
      81 candidates $\to$ best $(v_x, v_y)$\\
      {\tiny speed + heading}};

    \node[proc] (params) at (12,0) {Set CasADi Params\\
      $\alpha_i$, $m_{\text{scene}}$, $\nabla V$,\\
      $\hat{v}_{\text{ref}}$, $\hat{\theta}_{\text{ref}}$};

    % Row 3: Solve
    \node[proc, line width=1.2pt, minimum width=5cm,
          fill=mpcred!15] (ipopt) at (6,-3) {\textbf{IPOPT Solve}\\
      $\min_{v_{0:N},\, \omega_{0:N}} J(v, \omega; \text{params})$\\
      s.t.\ dynamics + safety constraints};

    \node[proc, fill=vlmgreen!10, draw=vlmgreen!60] (apply) at (12,-3) {Apply\\
      $v_0^*, \omega_0^*$\\
      $\to$ \texttt{/cmd\_vel}};

    % Arrows
    \draw[arrow, sarlblue!60] (grad) -- (params);
    \draw[arrow, sarlblue!60] (ref) -- (params);
    \draw[arrow] (params) |- (ipopt);
    \draw[arrow, very thick, vlmgreen!70] (ipopt) -- (apply);

    % Warm start feedback
    \draw[arrow, gray!60, dashed] (apply.south) -- ++(0,-0.8) -| (ipopt.south)
      node[midway, below, font=\tiny] {warm start from previous solution};

    % Timing annotation
    \node[draw=gray!40, fill=gray!5, rounded corners=3pt,
          font=\tiny, text width=4cm, align=center] at (0,-3) {
      Receding horizon at 10\,Hz.\\
      Gradient eval: ${\sim}1$\,ms (5 states).\\
      IPOPT solve: ${\sim}5$--$15$\,ms.\\
      If SARL stale $\to$ weights set to 0.
    };
  \end{tikzpicture}
\end{frame}
...
% ============================================================
\begin{frame}{SARL Bridge Node: Real-Time Inference + Action Selection}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{Triple Interface Design:}

      \vspace{4pt}
      \begin{block}{\footnotesize Interface 1: Topic Publisher (10\,Hz)}
        \begin{itemize}
          \item Content: $\{\alpha_i\}$, $V(s)$, person IDs
          \item \textbf{+ Recommended action} $(\hat{v}_x, \hat{v}_y)$
          \item RViz markers: attention spheres + action arrow
        \end{itemize}
      \end{block}
      ...
      \begin{block}{\footnotesize Interface 2: Batch Service}
        \begin{itemize}
          ...
          \item Used for terminal $\nabla V(s)$: 5 states
          \item Latency: $<1$\,ms (5 states)
        \end{itemize}
      \end{block}

      \begin{block}{\footnotesize Interface 3: Action Selection}
        \begin{itemize}
          \item 81 candidates: $5 \times 16 + 1$ (stop)
          \item $Q = r + \gamma^{\Delta t \cdot v_{\text{pref}}} V(s')$
          \item Single batched forward pass (${\sim}1$\,ms)
          \item Matches CrowdNav \texttt{cadrl.py} exactly
        \end{itemize}
      \end{block}
    \end{column}

    \begin{column}{0.48\textwidth}
      \textbf{Action Space} {\footnotesize (from CrowdNav)}:
      \[
        v_i = \frac{e^{(i+1)/5} - 1}{e - 1}\cdot v_{\text{pref}}, \quad
        \theta_j = \frac{2\pi j}{16}
      \]
      {\footnotesize 5 exponential speeds $\times$ 16 rotations + stop = 81}

      \vspace{4pt}
      \textbf{Q-Value Computation:}
      \begin{itemize}
        \item Propagate robot \& humans one step ($\Delta t = 0.25$\,s)
        \item Reward: collision $-0.25$, goal $+1.0$, proximity penalty
        \item Batch $V(s')$ via single forward pass
        \item Best action $\to$ $(v_x, v_y)$ in world frame
      \end{itemize}

      \vspace{4pt}
      \textbf{RViz Visualizations:}
      \begin{itemize}
        \item \texttt{/sarl/attention\_markers}: spheres, $V(s)$ text, \textbf{action arrow} (cyan)
        \item \texttt{/sarl/mpc\_markers}: terminal state, $\nabla V$ arrow, ref heading, MPC cmd
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Key Configuration Parameters}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{\footnotesize MPC Parameters (\texttt{mpc\_casadi\_sarl.yaml})}
        \footnotesize
        \begin{tabular}{lrl}
          \toprule
          ...
          $\Delta t$        & 0.2     & s \\
          $N$ (horizon)     & 15      & steps \\
          Solver            & \multicolumn{2}{l}{CasADi/IPOPT} \\
          Control rate      & 10      & Hz \\
          \midrule
          $w_{\text{goal}}$    & 20.0 & --- \\
          $w_{\text{social}}$  & 1.0  & --- \\
          ...
          $w_{\text{sarl-term}}$      & 2.0 & --- \\
          $w_{\text{sarl-ref-spd}}$   & 0.5 & --- \\
          $w_{\text{sarl-ref-hdg}}$   & 1.0 & --- \\
          \texttt{sarl\_ref\_horizon} & 3   & steps \\
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{exampleblock}{\footnotesize Launch Command}
        ...
        \quad mpc\_casadi\_sarl\_vlm.launch.py \\
...
\scriptsize
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{algorithmic}[1]
        \footnotesize
        \REQUIRE Trained SARL model, VLM API, CasADi MPC
        \STATE \textbf{Build} CasADi symbolic graph (once)
        \STATE \textbf{Initialize} ROS\,2 nodes, load SARL weights
        \WHILE{navigation active (at 10\,Hz)}
          \STATE Get robot state $(x, y, \theta, v)$ from TF
          \STATE Get pedestrian states $\lbrace\mathbf{p}_i, \mathbf{v}_i\rbrace$
          \STATE \textcolor{sarlblue}{\textbf{// SARL Inference (10\,Hz)}}
          \STATE Compute $\lbrace\alpha_i\rbrace, V(s)$ via ValueNetwork
          \STATE Select best action from 81 candidates \hfill \textit{Enh.\,C}
          \STATE Publish \texttt{/sarl/output} with $(\hat{v}_x, \hat{v}_y)$
          \STATE \textcolor{vlmgreen}{\textbf{// VLM (event-triggered)}}
          \IF{trigger condition met}
            \STATE Query VLM with SARL $\alpha_i$ \hfill \textit{Syn.\,1}
          \ENDIF
          \STATE \textcolor{mpcred}{\textbf{// CasADi MPC Optimization}}
          \STATE Compute $\nabla V(s_N)$ via 5-pt finite diff. \hfill \textit{Enh.\,A}
          \STATE Set CasADi params: $\alpha_i, m, \nabla V, \hat{v}, \hat{\theta}$
          \STATE IPOPT solve $\to$ $(v^*_{0:N}, \omega^*_{0:N})$
          \STATE Apply $v_0^*, \omega_0^*$ to \texttt{/cmd\_vel}
          \STATE Publish explanation + markers \hfill \textit{Syn.\,3}
        \ENDWHILE
      \end{algorithmic}
    \end{column}
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{Long-Term}
        \begin{itemize}
          ...
          \item Online SARL fine-tuning from MPC experience
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
  ...
  \begin{columns}[T]
    \begin{column}{0.32\textwidth}
      \begin{block}{\footnotesize SARLOutput.msg}
        ...
        float32 robot\_y\\
        float32 robot\_yaw\\
        \textcolor{sarlblue}{float32 recommended\_vx}\\
        \textcolor{sarlblue}{float32 recommended\_vy}\\
        \textcolor{sarlblue}{float32 recommended\_speed}\\
        \textcolor{sarlblue}{float32 recommended\_heading}\\
        \textcolor{sarlblue}{bool has\_recommended\_action}\\
